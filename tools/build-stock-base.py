...
class Builder:
    def __init__(self, args):
        ...
        self.h = load('gold_hybrid', REPO / 'sources/device/xiaomi/gold/tools/hybrid_properties.py')
    ...
    def vendor(self, stage):
        patch = REPO / 'integration/stock/vendor-compat.patch'
        ...
        (stage / CACHE).unlink()
        return VENDOR_CHANGES, {CACHE}

    def mi_ext(self, stage):
        # Xiaomi's empty placeholder masks Lineage's real Messaging APK.
    ...
    def run(self):
        ...
        lock = json.loads((REPO / 'firmware/gold-cn.json').read_text())
        ...
        for part in ('system', 'system_ext', 'product', 'vbmeta_system', 'odm'):
            ...
            (self.images / source.name).symlink_to(source)
        ...
        fec = {part: self.fec(part) for part in ('vendor', 'mi_ext')}
        images = {p.name: {'size': p.stat().st_size, 'sha256': sha(p)}
                  for p in sorted(self.images.glob('*.img'))}
        report = {'stock_version': lock['version'], 'lineage_inputs': lineage_inputs, 'images': images,
...
def main():
    ...
    for name in ('source', 'stock', 'lineage_images', 'output', 'lock'):
