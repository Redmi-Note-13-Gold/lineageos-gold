...
def validate(roots, native, work, label):
    ...
    native.run("secilc", *policy_files(roots), "-m", "-M", "true", "-G",
...
def resign(avbtool, source, replacement, key, output, partition_name="system_ext"):
    ...
    new = partition_descriptor(avbtool, parse_avb(avbtool, replacement)[2], partition_name)
    old = partition_descriptor(avbtool, descriptors, partition_name)
